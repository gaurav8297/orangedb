...
// Timing utility macros for performance debugging
...
#define TIMER_END(name) \
    ...
    auto timer_##name##_duration = std::chrono::duration_cast<std::chrono::milliseconds>(timer_##name##_end - timer_##name##_start).count(); \
    printf("<== EXITING: %s (Duration: %.3f seconds)\n\n", #name, timer_##name##_duration / 1000.0); \
    fflush(stdout);
...
// Function to get L1 cluster sizes from L1_ClusterVectorIds
std::vector<size_t> get_l1_cluster_sizes(
    const std::vector<std::vector<vector_idx_t>>& L1_ClusterVectorIds) {
    
    std::vector<size_t> cluster_sizes(L1_ClusterVectorIds.size());
    
    // Parallelize size computation across L1 clusters
    #pragma omp parallel for schedule(dynamic, 16)
    for (size_t miniId = 0; miniId < L1_ClusterVectorIds.size(); miniId++) {
        cluster_sizes[miniId] = L1_ClusterVectorIds[miniId].size();
    }
    
    return cluster_sizes;
}

// Function to get L2 cluster sizes from L2_ClusterVectorIds
std::vector<size_t> get_l2_cluster_sizes(
    const std::vector<std::vector<vector_idx_t>>& L2_ClusterCentroidIds) {
    
    std::vector<size_t> cluster_sizes(L2_ClusterCentroidIds.size());
    
    // Parallelize size computation across L1 clusters
    #pragma omp parallel for schedule(dynamic, 16)
    for (size_t megaId = 0; megaId < L2_ClusterCentroidIds.size(); megaId++) {
        cluster_sizes[megaId] = L2_ClusterCentroidIds[megaId].size();
    }
...
double get_recall(ReclusteringIndex &index, float *queryVecs, size_t queryDimension, size_t queryNumVectors, int k,
    ...
    // search
    double recall = 0;
    ReclusteringIndexStats localStats;
    ReclusteringIndexStats& stats = externalStats ? *externalStats : localStats;
    double max_recall = 0;
...
void benchmark_fixed_recall(InputParser &input) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        // make sure the probe number is correct
        ReclusteringIndexStats iterStats1;
        TIMER_START(iter_verify_recall_loop_1);
        for (int j = 0; j < queryNumVectors; j++) {
            float *query = queryVecs + j * queryDimension;
            std::vector<double> singleQueryRecall;
            printf("Query %d: ", j);
        ...
        TIMER_END(iter_verify_recall_loop_1);
...
double get_recall(IncrementalIndex &index, float *queryVecs, size_t queryDimension, size_t queryNumVectors, int k,
    ...
    if (queryNumVectors == 1){
        printf("Recall: %f, nL2Probes: %d, nL1Probes: %d, distance computations: %llu\n", recall, nMegaProbes, nMicroProbes, stats.numDistanceComp);
    } else{
...
std::vector<std::pair<int, int>> get_fixed_recall(ReclusteringIndex &index, float *queryVecs, size_t queryDimension, size_t queryNumVectors, int k,
    ...
    //TIMER_START(get_fixed_recall_get_centroids);
    const float *l1_centroids = nullptr;
    size_t num_l1_centroids = 0;
    index.getMiniCentroids(&l1_centroids, num_l1_centroids);
    const float *l2_centroids = nullptr;
    size_t num_l2_centroids = 0;
    index.getMegaCentroids(&l2_centroids, num_l2_centroids);
    //TIMER_END(get_fixed_recall_get_centroids);

    // OPTIMIZATION: Parallelize query processing with OpenMP
    //TIMER_START(get_fixed_recall_query_loop);
    ...
    //TIMER_END(get_fixed_recall_query_loop);
    
    //TIMER_END(get_fixed_recall);
    return nProbes;
}
