...
namespace orangedb {
    ...
    public:
        Clustering(int dim, int dataDim, int numCentroids, int nIter, int minCentroidSize, int maxCentroidSize,
                   DelegateDC<T> *dc = createDistanceComputer(nullptr, dim, 0, DistanceType::L2),
                   std::function<float(T)> decodeFunc = [](float a) {return a;}, float lambda = 0);
    ...
    private:
        ...
        std::function<float(T)> decodeFunc;
